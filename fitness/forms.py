...
class UserProfileForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "goal_event": forms.TextInput(
                attrs={"placeholder": "E.g., 5K run, Marathon"}
            ),
        ...
        if goal_date:
            ...
            if goal_date <= today:
                raise ValidationError(
                    "The target date must be set for a date in the future."
                )
            # Must be within the next 12 months
            if goal_date > one_year_from_now:
                raise ValidationError(
                    "The target date cannot be more than one year away."
                )
        return goal_date


class TrainingPlanForm(forms.ModelForm):
    """Form for creating or editing training plans."""
...
class PlanGenerationForm(forms.ModelForm):
    ...
    last_plan_feedback = forms.CharField(
        ...
        help_text=(
            "Provide any feedback or comments " "on your previous training plan."
        ),
    )
    plan_preferences = forms.CharField(
        ...
        widget=forms.Textarea(attrs={"rows": 3}),
        help_text=(
            "Any specific preferences or requirements " "for your new training plan."
        ),
    )
